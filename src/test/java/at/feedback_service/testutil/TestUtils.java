...
public class TestUtils {
    ...
    public static void compareCourseInfos(List<Rating> ratings, List<CourseRatingInfo> actual) {
        assertThat(actual.size()).isEqualTo(ratings.size());
        var sortedActual = actual.stream().sorted(Comparator.comparing(CourseRatingInfo::getCourseId)).toList();
        for (int i = 0; i < sortedActual.size(); i++) {
            compareCourseInfo(ratings.get(i), sortedActual.get(i));
        }
    }

    public static void compareCourseInfo(Rating rating, CourseRatingInfo actual) {
        CourseRatingInfo expected = calculateWilsonScoreAvgStars(rating);
        assertThat(actual).isEqualTo(expected);
    }
    // check if trigger works in the certain moments
    public static CourseRatingInfo calculateWilsonScoreAvgStars(Rating rating) {
        ...
        return new CourseRatingInfo(rating.getCourseId(), wilsonScore, avgStars);
    }
